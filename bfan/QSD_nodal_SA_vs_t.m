...
for ss=1:N_sample
    ...
    for tt=2:length(ts)
        ...
        D=mat_phi*mat_phi';
        S_A_t(tt) = entanglement_entroy_D(D,subA);
    end
    SA_t(:,ss) = S_A_t;
   
end
file_name = sprintf('QSD_nodal_L%d_gm%.2f_Nsam%d_SA_t_test.mat',L,gamma,N_sample);
save(file_name,'L','N','N_sample','gamma','SA_t','ts','-v7.3')
%save(file_name,'-v7.3')
